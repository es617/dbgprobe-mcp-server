...
from dbgprobe_mcp_server.plugins import PluginManager
...
TOOLS: list[Tool] = [
    Tool(
        ...
        description=(
            ...
            "Plugins require DBGPROBE_MCP_PLUGINS env var — set to 'all' for all or 'name1,name2' to allow specific plugins. "
    ...
    Tool(
        name="dbgprobe.plugin.reload",
        description=(
            "Hot-reload a plugin by name. Re-imports the module and refreshes tools. "
            "Requires DBGPROBE_MCP_PLUGINS env var to be set."
        ),
        inputSchema={
            "type": "object",
            "properties": {
    ...
    Tool(
        ...
        description=(
            ...
            ".dbgprobe_mcp/plugins/<name>.py, fill in the tools and handlers, "
            "then load with dbgprobe.plugin.load."
        ),
        inputSchema={
            "type": "object",
            "properties": {
...
def make_handlers(manager: PluginManager, server: Server) -> dict[str, Any]:
    ...
    async def handle_plugin_template(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
    ...
    async def handle_plugin_list(_state: ProbeState, _args: dict[str, Any]) -> dict[str, Any]:
        plugins = [
    ...
    async def handle_plugin_reload(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
        name = args.get("name", "")
        if not name:
    ...
    async def handle_plugin_load(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "dbgprobe.plugin.template": handle_plugin_template,
        "dbgprobe.plugin.list": handle_plugin_list,
        "dbgprobe.plugin.reload": handle_plugin_reload,
        "dbgprobe.plugin.load": handle_plugin_load,
    }
