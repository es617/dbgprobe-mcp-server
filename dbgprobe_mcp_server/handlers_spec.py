...
from dbgprobe_mcp_server import specs
...
TOOLS: list[Tool] = [
    Tool(
        name="dbgprobe.spec.template",
        description=(
            "Return a markdown template for a new debug probe protocol spec. Optionally pre-fill with a device name."
        ),
        inputSchema={
            "type": "object",
            "properties": {
    ...
    Tool(
        name="dbgprobe.spec.register",
        description=(
            "Register a spec file in the index. Validates YAML front-matter "
            "(requires kind: dbgprobe-protocol and name). The file path can be "
    ...
    Tool(
        name="dbgprobe.spec.attach",
        description=(
            "Attach a registered spec to a connection session (in-memory only). "
            "The spec will be available via dbgprobe.spec.get for the duration of this connection. "
            "After attaching, check dbgprobe.plugin.list for a matching plugin, then present "
            ...
            "or create a new plugin using dbgprobe.plugin.template."
        ),
        inputSchema={
            "type": "object",
            "properties": {
...
async def handle_spec_template(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
...
async def handle_spec_register(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
    path = args["path"]
    try:
...
async def handle_spec_list(_state: ProbeState, _args: dict[str, Any]) -> dict[str, Any]:
...
async def handle_spec_search(_state: ProbeState, args: dict[str, Any]) -> dict[str, Any]:
    spec_id = args["spec_id"]
    query = args["query"]
    k = int(args.get("k", 10))
    try:
...
HANDLERS: dict[str, Any] = {
    "dbgprobe.spec.template": handle_spec_template,
    "dbgprobe.spec.register": handle_spec_register,
    "dbgprobe.spec.list": handle_spec_list,
    "dbgprobe.spec.attach": handle_spec_attach,
    "dbgprobe.spec.get": handle_spec_get,
    "dbgprobe.spec.read": handle_spec_read,
    "dbgprobe.spec.search": handle_spec_search,
}
