"""Plugin loader and manager for Debug Probe MCP server.

Loads user plugins from `.dbgprobe_mcp/plugins/`. No serial imports.
"""
...
def parse_plugin_policy() -> tuple[bool, set[str] | None]:
    """Parse ``DBGPROBE_MCP_PLUGINS`` env var.
    ...
    """
    raw = os.environ.get("DBGPROBE_MCP_PLUGINS", "").strip()
    if not raw:
        return False, None
    if raw in ("*", "all"):
        return True, None
...
def load_plugin(plugin_path: Path) -> tuple[str, list[Tool], dict[str, Any], str, dict[str, Any]]:
    ...
    module_key = f"dbgprobe_mcp_plugin__{name}__{path_hash}"
    ...
    def _check_allowed(self, name: str) -> None:
        ...
        if not self.enabled:
            raise PermissionError(
                "Plugins are disabled. Set DBGPROBE_MCP_PLUGINS=all or DBGPROBE_MCP_PLUGINS=name1,name2 to enable."
            )
        if self.allowlist is not None and name not in self.allowlist:
            raise PermissionError(
                f"Plugin '{name}' is not in the allowlist (DBGPROBE_MCP_PLUGINS={self.policy})."
            )
